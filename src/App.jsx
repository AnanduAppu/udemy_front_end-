    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.post(
          "http://localhost:4001/user/useraccess",
          {
            email: id,
          }
        );
    ...
    async function fetchCourses() {
      try {
        const response = await axios.get(
          "http://localhost:4001/user/allcourses"
        );
    ...
    async function fetchCourses() {
      try {
        ...
        const backendResponse = await axios.get(
          `http://localhost:4001/user/usercourses`,
          {
            headers: {
              id: id,
            },
  ...
  useEffect(() => {
    const fetchCart = async () => {
      try {
        ...
        const backendResponse = await axios.get(
          `http://localhost:4001/user/cartShow/${id}`
        );
        ...
        if (backendResponse.data.successful) {
          ...
          setCartData(
            cartData.length === 0 ? backendResponse.data.Data : cartData
          );
          setCartStatus(
            cartStaus === null ? backendResponse.data.additional : cartStaus
          );
        }
      } catch (error) {
        toast.error("failed");
  ...
  useEffect(() => {
    const fetchWish = async () => {
      try {
        ...
        const backendResponse = await axios.get(
          `http://localhost:4001/user/wishlistshow/${id}`
        );
        ...
        if (backendResponse.data.successful) {
          ...
          setWishData(
            WishData.length === 0 ? backendResponse.data.Data : WishData
          );
          setWishStatus(
            WishStaus === null ? backendResponse.data.additional : WishStaus
          );
        }
      } catch (error) {
        toast.error("failed");
      ...
      <Clintcontex.Provider value={data}>
        <Routes>
          <Route path="/" element={<Assemble />}>
            ...
            <Route
              path="/login/emailvarify/resetpass"
              element={<ResetPass2 />}
            />

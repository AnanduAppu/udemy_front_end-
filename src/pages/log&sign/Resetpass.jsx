...
function Resetpass() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Perform any additional logic with the email value, e.g., send a reset password request
    console.log('Email:', email); 
    if(email.length===0){
  
        alert("enter email in the input")

    }else{
        axios.post("http://localhost:4001/user/emailvarification",{email})
        .then((res)=>{ 

            console.log(res)
            navigate("/login/emailvarify/resetpass")
